...
model = tf.keras.models.load_model(
    r"C:\Users\surya\Desktop\Projects\zf_python\junction_box\new\trained_model2.h5")
model_wire = tf.keras.models.load_model(
    r"C:\Users\surya\Desktop\Projects\zf_python\junction_box\new\trained_model_wire.h5")
...
img = cv2.imread(
    r'C:\Users\surya\Desktop\Projects\object_detection\wire_data\IMG_20201223_120539.jpg')
img3 = img.copy()
...
for i in range(len(sbg)):
    for j in range(len(white_boxes)):
        ...
        if aoi == 1:
            x1, y1, x2, y2 = bbbox(white_boxes[j], sbg[i][1])
            x1 = int((x1/500)*4000)
            x2 = int(((x2)/500)*4000)
            y1 = int((y1/500)*1824)
            y2 = int(((y2)/500)*1824)
            cv2.rectangle(img3, (x1, y1), (x2, y2), (255, 0, 0), 4)
            print(white_num[j])
