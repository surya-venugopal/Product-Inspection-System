...
num_model = r"C:\Users\surya\Desktop\Projects\ZF\junction_box\akash\trained_model.h5"
wire_model = r"C:\Users\surya\Desktop\Projects\ZF\junction_box\akash\new_trained_wire.h5"

temp_img_path = r"C:\Users\surya\Desktop\Projects\ZF\junction_box\akash\temp\t\trmp.jpg"
temp_img_folder = r"C:\Users\surya\Desktop\Projects\ZF\junction_box\akash\temp"


model = tf.keras.models.load_model(num_model)
model_wire = tf.keras.models.load_model(wire_model)
...
def predict(img):
    ...
    gen = tf.keras.preprocessing.image.ImageDataGenerator(
        rescale=1/255., validation_split=0.05)
    train = gen.flow_from_directory(temp_img_folder, target_size=(
        28, 28), subset="training", batch_size=16, color_mode="grayscale")
    x, y = train.next()
...
while True:
    ...
    if k % 256 == 27:
        ...
        m2 = mask+mask1
        contours, hierarchy = cv2.findContours(
            m2, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        light_white = (180, 200, 200)
        dark_white = (255, 255, 255)
        mask5 = cv2.inRange(img, light_white, dark_white)
        contours, hierarchy = cv2.findContours(
            mask5, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        white_boxes = []
        white_num = []
