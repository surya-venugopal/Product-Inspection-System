...
from PyQt5.QtWidgets import QWidget,QMessageBox, QAction, QSplashScreen
...
#         self.imageV = cv2.resize(self.imageV,(int(self.screenWidth *0.4),int(self.screenHeight*0.4)))
        ...
        step = channel * width
        qImg = QImage(self.imageV.data, width, height, step, QImage.Format_RGB888)
        self.ui.capturedImage.setPixmap(QPixmap.fromImage(qImg))
        self.isCaptured = True
        self.capturedImage = self.image.copy()
...
#         self.image = self.image.tobytes('raw', 'RGB')
        ...
        self.imageV = cv2.resize(self.image,(int(self.screenWidth/2),int(self.screenHeight/2)))
        ...
        if(self.attempt < 4):
            ...
            if(result == True ):
                ...
                if(password == "surya"):
                    ...
                    df.to_excel('/home/zf_admin/Desktop/ZF_Inspection_{}.xlsx'.format(self.date),index=False)
                    try:
                        df.to_excel('/home/zf_admin/Desktop/ZF_Inspection_{}.xlsx'.format(self.date),index=False)
                    except:
