                            ...
                            try:
                                if(self.suffix != 0):
                                    path, dirs, files = next(os.walk(
                                        self.db_path_ins+"DB/{date}/{serialNo}_{suff}".format(date=self.model, serialNo=self.serialNo, suff=self.suffix)))
                                else:
                                    path, dirs, files = next(os.walk(
...
#                         self.camera.exit()
    ...
    def save(self):
        if self.isCaptured:
            # cv2.imwrit.e(self.path+"DB/{date}/{serialNo}/".format(date=self.date, serialNo=self.serialNo) + time + "_" + str(self.i)+".jpg", self.image)
            if(self.i == 1 and os.path.exists(
                    self.db_path_ins+"DB/{date}/{serialNo}/".format(date=self.model, serialNo=self.serialNo))):
                while 1:
